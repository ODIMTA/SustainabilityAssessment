function [strE,strS,strM,str,MediaISE,DesvISE,VarISE,MinimoISE,MaximoISE,MediaISS,DesvISS,VarISS,MinimoISS,MaximoISS,MediaISM,DesvISM,VarISM,MinimoISM,MaximoISM,MediaIS,DesvIS,VarIS,MinimoIS,MaximoIS,ISE,ISS,ISM,IS]=CocheElecProbabilistaCorrelacionado_28_03_2025_PERT
...
Rho4=[1 0.45 0.4; 0.45 1 0.8; 0.4 0.8 1];
vv=copularnd('Gaussian',Rho4,n);
[aSI1,bSI1]=betaPERT(7.5,10,10);
[aSI2,bSI2]=betaPERT(4,9.4,10);
[aSI3,bSI3]=betaPERT(0,9.5,10);
pdSI1=makedist('beta',aSI1,bSI1);
pdSI2=makedist('beta',aSI2,bSI2);
pdSI3=makedist('beta',aSI3,bSI3);

XY=[icdf(pdSI1,vv(:,1)) icdf(pdSI2,vv(:,2)) icdf(pdSI3,vv(:,3))];
XYY(:,1)=XY(:,1)*(10-7.5)+7.5;
XYY(:,2)=XY(:,2)*(10-4)+4;
XYY(:,3)=XY(:,3)*(10-0)+0;

for i=1:n
   V_SI1(i)=FVcontinua(XYY(i,1),0,10,1,0.01,1,"creciente");
   V_SI2(i)=FVcontinua(XYY(i,2),0,10,1,0.01,1,"creciente");
   V_SI3(i)=FVcontinua(XYY(i,3),0,10,1,0.01,1,"creciente");   
end
% Comprobación correlaciones
...
for i=1:n
    ...
    ISM(i)=PCriterioMedioambiental*(PSI1*V_SI1(i)+PSI2*V_SI2(i)+PSI3*V_SI3(i));
