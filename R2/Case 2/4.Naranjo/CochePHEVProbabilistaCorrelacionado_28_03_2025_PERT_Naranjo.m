function [strE,strS,strM,str,MediaISE,DesvISE,VarISE,MinimoISE,MaximoISE,MediaISS,DesvISS,VarISS,MinimoISS,MaximoISS,MediaISM,DesvISM,VarISM,MinimoISM,MaximoISM,MediaIS,DesvIS,VarIS,MinimoIS,MaximoIS,ISE,ISS,ISM,IS]=CochePHEVProbabilistaCorrelacionado_28_03_2025_PERT_Naranjo
...
[aPCOM,bPCOM]=betaPERT(35000,50000,170000);
[aCMR,bCMR]=betaPERT(350,550,1800);
[aCITV,bCITV]=betaPERT(160,350,1300);
...
Y(:,1)=X(:,1)*(170000-35000)+35000;
Y(:,2)=X(:,2)*(1800-350)+350;
Y(:,3)=X(:,3)*(1300-160)+160;
...
[aSOA,bSOA]=betaPERT(83,85,93);
[aSON,bSON]=betaPERT(83,88,88);
[aUVC,bUVC]=betaPERT(70,82,84);
[aASC,bASC]=betaPERT(67,79,80);
...
[aAUT,bAUT]=betaPERT(500,850,1200);
pdAUT=makedist('beta',aAUT,bAUT);
AUT_aux=random(pdAUT,1,n);
AUT=AUT_aux*(1200-500)+500;
...
[aPM,bPM]=betaPERT(0.11,0.20,0.38);
[aPP,bPP]=betaPERT(0.090,0.130,0.300);
pdPM=makedist('beta',aPM,bPM);
pdPP=makedist('beta',aPP,bPP);
MM=[icdf(pdPM,w(:,1)) icdf(pdPP,w(:,2))];
MMM(:,1)=MM(:,1)*(0.38-0.11)+0.11;
MMM(:,2)=MM(:,2)*(0.300-0.090)+0.090;
for i=1:n
...
pdSI1=makedist('uniform',0.212,0.234);
SI1=random(pdSI1,1,n);
SI2=1.24E-7;
SI3=0.0084;
SI4=7.67E-4;
pdSI5=makedist('uniform',2.53E-4,3.36E-4);
SI5=random(pdSI5,1,n);
SI6=6.16E-4;
SI7=8.63E-5;
SI8=4.94E-6;
pdSI9=makedist('uniform',0.0131,0.045);
SI9=random(pdSI9,1,n);
pdSI10=makedist('uniform',7.54E-3,0.0333);
SI10=random(pdSI10,1,n);
pdSI11=makedist('uniform',0.102,1.163);
SI11=random(pdSI11,1,n);
SI12=0.0067;
SI13=0.0018;
SI14=0.07;
SI15=0.001;
...
for i=1:n
   V_SI1(i)=FVcontinua(SI1(i),0.27,0.1,0.253,0.01,1,"decreciente");
   V_SI2(i)=FVcontinua(SI2,1.45E-7,6.95E-8,1.37E-7,0.01,1,"decreciente");
   V_SI3(i)=FVcontinua(SI3,0.03,0.0065,0.02765,0.01,1,"decreciente"); 
   V_SI4(i)=FVcontinua(SI4,2E-3,0.00077,0.001877,0.01,1,"decreciente");
   V_SI5(i)=FVcontinua(SI5(i),0.0004,0.000205,0.0003805,0.01,1,"decreciente");
   V_SI6(i)=FVcontinua(SI6,8.5E-4,0.00053,0.000818,0.01,1,"decreciente");
   V_SI7(i)=FVcontinua(SI7,0.00015,0.000065,0.0001415,0.01,1,"decreciente");
   V_SI8(i)=FVcontinua(SI8,1E-5,0.0000042,0.00000942,0.01,1,"decreciente");
   V_SI9(i)=FVcontinua(SI9(i),0.078,0.013,0.0715,0.01,1,"decreciente");
   V_SI10(i)=FVcontinua(SI10(i),4.5E-2,0.007,0.412,0.01,1,"decreciente");
   V_SI11(i)=FVcontinua(SI11(i),2.5,0.09,2.259,0.01,1,"decreciente");
   V_SI12(i)=FVcontinua(SI12,8.5E-3,0.006,0.00825,0.01,1,"decreciente");
   V_SI13(i)=FVcontinua(SI13,0.0025,0.0014,0.00239,0.01,1,"decreciente");
   V_SI14(i)=FVcontinua(SI14,8E-2,0.03,0.075,0.01,1,"decreciente");
   V_SI15(i)=FVcontinua(SI15,0.0013,0.001,0.00127,0.01,1,"decreciente");
end
% Comprobación correlaciones
